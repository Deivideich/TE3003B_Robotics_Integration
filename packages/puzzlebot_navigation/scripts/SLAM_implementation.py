...
class MCLNode(Node):
    ...
    def ts_map_init(self):
        self.get_logger().info("Initializing SLAM map")
    ...
    def scan_to_coord(self, scan_angles, scan_ranges, theta):
        ...
        y_slam = np.zeros(len(scan_ranges))
        for i in range(len(scan_ranges)):

            x = scan_ranges[i] * np.cos(scan_angles[i] + theta)
            y = scan_ranges[i] * np.sin(scan_angles[i] + theta)
            x_slam[i] = x
            y_slam[i] = y
        return x_slam, y_slam
    
    def clamp_values(self, slam_ranges):
        for i in range(len(slam_ranges)):
            if not np.isfinite(slam_ranges[i]):
                slam_ranges[i] = self.MAX_RANGE
        return slam_ranges
    
    def ts_map_update(self, quality):
        def get_pose(odom):
            pos = odom.pose.pose.position
            ori = odom.pose.pose.orientation
            _, _, yaw = tf_transformations.euler_from_quaternion([ori.x, ori.y, ori.z, ori.w])
        ...
        max_range = int(self.scan.range_max)
        
        scan_ranges = self.clamp_values(scan_ranges)
        # self.get_logger().info(f"scan_ranges: {scan_ranges}")
        slam_points = len(scan_ranges)
        ...
        self.mcl_cpp.ts_map_update.argtypes = [
            ctypes.c_int,                      # x1
            ctypes.c_int,                      # x2
            ctypes.c_int,                    # slam points (n_points)
            ctypes.POINTER(ctypes.c_float),                    # scan ranges (float array)
            ctypes.c_int,    # max range
            ctypes.POINTER(ctypes.c_int),    # map_array (uint8_t array)
            ...
            ctypes.c_int,                    # TS_NO_OBSTACLE
            ctypes.c_int,                    # TS_OBSTACLE
            ctypes.c_int                    # TS_MAP_SIZE
        ]
        self.mcl_cpp.ts_map_update.restype = ctypes.c_bool

        # Convert numpy arrays to ctypes
        scan_ranges_ctypes = scan_ranges.ctypes.data_as(ctypes.POINTER(ctypes.c_float))
        slam_map_ctypes = self.slam_map.ctypes.data_as(ctypes.POINTER(ctypes.c_int))
        x_slam_ctypes = x_slam.ctypes.data_as(ctypes.POINTER(ctypes.c_double))
        y_slam_ctypes = y_slam.ctypes.data_as(ctypes.POINTER(ctypes.c_double))


        # Call the function
        success = self.mcl_cpp.ts_map_update(
            x1,
            y1,
            slam_points,
            scan_ranges_ctypes,
        ...
        if success:
            ...
            self.get_logger().warn("C++ SLAM map update failed. Falling back to Python version.")
    ...
    def save_slam_map_periodically(self):
        ...
        if self.last_saved_map is None or not np.array_equal(current_map, self.last_saved_map):
            ...
            # Save PGM
            pgm_path = os.path.join(save_dir, 'slam_map.pgm')
            imageio.imwrite(pgm_path, current_map.reshape((self.TS_MAP_SIZE, self.TS_MAP_SIZE)).astype(np.uint8))        
            
            # Save YAML with origin and resolution
            yaml_path = os.path.join(save_dir, 'slam_map.yaml')
            ...
            self.get_logger().info(f"SLAM map saved to {pgm_path} and {yaml_path}")
            self.last_saved_map = current_map.copy()  
